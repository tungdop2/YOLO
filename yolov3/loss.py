...
def post_process_output(output, ANCHOR_BOXS=cfg.ANCHOR_BOXS[0], device="cpu"):
    BOX = cfg.BOX
    # xy
...
def custom_loss(outputs, targets, ANCHOR_BOXS=cfg.ANCHOR_BOXS[0], device="cpu"):
    lambda_coord = 1.0
    lambda_noobj = 0.5
    lambda_obj = 5.0
    lambda_cls = 1.0

    pred_xy, pred_wh, pred_obj, pred_cls = post_process_output(
        outputs, ANCHOR_BOXS, device=device)
