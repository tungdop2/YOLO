...
def detect(weights='yolov3.pt', path='datasets/val/images/', save_path='detect/yolov3/', save_txt=False):
    S = cfg.S
    BOX = cfg.BOX
    CLS = cfg.CLS
    H, W = cfg.H, cfg.W
    ...
    with torch.no_grad():
        for img in os.listdir(path):
            ...
            bboxes = outputs_tensor_to_bbox(outputs, OUTPUT_THRESH)
            ...
            for bbox in bboxes:
                ...
                y = y - h / 2
                x, y, w, h = x * \
                    raw.shape[1], y * raw.shape[0], w * \
                    raw.shape[1], h * raw.shape[0]

                cv2.rectangle(raw, (int(x), int(y)),
                              (int(x + w), int(y + h)), (0, 255, 0), 2)
                cv2.putText(raw, '{:.2f}'.format(conf), (int(x), int(
                    y)), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
            cv2.imwrite(os.path.join(save_path, img_name), raw)
            print('Saved image to {}'.format(
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_path', type=str, default='detect/yolov3/')
